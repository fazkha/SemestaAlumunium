...
class MitraController extends Controller
{
    ...
    public function getTargetBonusList()
    {
        ...
        $target = MitraTargetBonus::where('isactive', 1)
            ->where('product_id', $product_id)
            ->selectRaw('id, target, bonus as name')
            ->get()
            ->toJson();
    ...
    public function saveKritikSaran(Request $request)
    {
        ...
        $kritiksaran = MitraKritikSaran::where('isactive', 1)
            ->where('product_id', $product_id)
            ->get();
        // where('user_id', $data['id'])
    ...
    public function loadKritikSaran(Request $request)
    {
        ...
        $kritiksaran = MitraKritikSaran::join('users', 'mitra_kritik_sarans.user_id', '=', 'users.id')
            ...
            ->where('mitra_kritik_sarans.product_id', $product_id)
    ...
    public function saveOmzet(Request $request)
    {
        ...
        //     'Authorization' => 'Bearer ' . env('TLM_TOKEN'),
        //     'Accept' => 'application/json',
        // ])->get(env('TLM_API') . '/order-mitra', [
        //     'branch_id' => $p1,
        //     'gerobak_id' => $p1,
        //     'tanggal' => $tanggal,
        // ]);
        ...
        if ($response->successful()) {
            ...
            if ($jenis) {
                if ($jenis->nama == 'Kasbon') {
                    ...
                    $app_plafon = AppSettingJualan::where('parm', 'mitra_kasbon_plafon')->first();
    ...
    public function loadOmzet(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'omzet' => $omzet ? strval($omzet->omzet) : '',
    ...
    public function hapusPengeluaran(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id' => ['required', 'integer', 'exists:users,id'],
            'tanggal' => ['required', 'date'],
            'keterangan' => ['required', 'string', 'exists:mysql2.jenis_pengeluaran_mitras,nama'],
        ]);
    ...
    public function loadOmzetPekanan(Request $request)
    {
        ...
        $omzet = DB::connection('mysql2')->select("CALL sp_mitra_omset_pekanan(?,?)", [$data['id'], $product_id]);
        $trend = null;
        $pct = null;
        $trend_bonus = null;
        $pct_bonus = null;
        ...
        if ($omzet) {
            ...
            $target_bonus = MitraAverageOmzet::join('mitra_target_bonuses', 'mitra_target_bonuses.id', '=', 'mitra_average_omzets.target_id')
                ...
                ->where('mitra_average_omzets.product_id', $product_id)
                ->first();
            ...
            if ($cOmzet >= 0) {
                ...
                $bonus = DB::connection('mysql2')->select("CALL sp_mitra_target_bonus(?,?)", [$cOmzet, $product_id]);
    ...
    public function saveTargetBonus(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'target_id' => ['required', 'integer', 'exists:mysql2.mitra_target_bonuses,id'],
        ]);
        ...
        $app_setting = AppSettingJualan::where('parm', 'mitra_pembagi_rata2_omzet')->first();
    ...
    public function loadBiayaHarian(Request $request)
    {
        ...
        $pendukungBiaya = DB::connection('mysql2')->select("CALL sp_omzetharianpc(?,?,?)", [$data['id'], $data['tanggal'], $product_id]);
        $biaya = DB::connection('mysql2')->select("CALL sp_mitra_pengeluaran_harian(?,?,?)", [$data['id'], $data['tanggal'], $product_id]);
    ...
    {
        $validator = Validator::make($request->all(), [
            'id' => ['required', 'integer', 'exists:mysql2.mitra_op_details,id'],
        ...
        if ($approve) {
            ...
            if ($pettyCash) {
                ...
                if ($approve->approved == 1) {
                    $nominal = $pengali * ($approve->harga ? $approve->harga : 0) * ($approve->jumlah ? $approve->jumlah : 0);
                }
                $pettyCash->update([
                    'nominal' => $nominal,
            ...
            $pendukungBiaya = DB::connection('mysql2')->select("CALL sp_omzetharianpc(?,?,?)", [$data['pc_id'], $data['tanggal'], $product_id]);
            $biaya = DB::connection('mysql2')->select("CALL sp_mitra_pengeluaran_harian(?,?,?)", [$data['pc_id'], $data['tanggal'], $product_id]);
        }
