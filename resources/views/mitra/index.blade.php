...
<x-app-layout>
    ...
    <div class="mx-auto px-4 py-2">
        <div class="flex flex-col items-center">
            ...
            <div class="w-full">
                @include('mitra.partials.filter')
            </div>

            <div id="table-container" class="w-full">
                @include('mitra.partials.table')
            </div>
            ...
            $("#pp-dropdown, #isactive-dropdown, #kelamin-dropdown, #cabang_id-dropdown, #jabatan_id-dropdown, #search-telpon, #search-nama, #search-alamat")
                .on(
                    ...
                    function() {
                        ...
                        var newURL = '{{ url('/human-resource/mitra') }}';
                        var newState = {
                            page: 'index-mitra'
                        };
                        var newTitle = '{{ __('messages.partner') }}';

                        window.history.pushState(newState, newTitle, newURL);

                        $.ajax({
                            url: '{{ url('/human-resource/mitra/fetchdb') }}' + "/" + xpp + "/" + xisactive + "/" +
