<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option
                                {{ session('mitra_pp') == config('custom.list_per_page_opt_1') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_1') }}">
                                {{ config('custom.list_per_page_opt_1') }}</option>
                            <option
                                {{ session('mitra_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option
                                {{ session('mitra_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700">
                            <option {{ session('mitra_isactive') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('mitra_isactive') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('mitra_isactive') == '0' ? 'selected' : '' }} value="0">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100">@lang('messages.gender')</span>
                        <select id="kelamin-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700">
                            <option {{ session('mitra_kelamin') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('mitra_kelamin') == '1' ? 'selected' : '' }} value="L">
                                @lang('messages.genderman')</option>
                            <option {{ session('mitra_kelamin') == '0' ? 'selected' : '' }} value="P">
                ...
                <div class="flex-row justify-start">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-nama" placeholder="@lang('messages.search')"
                            value="{{ session('mitra_nama_lengkap') == '_' ? '' : session('mitra_nama_lengkap') }}"
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100">@lang('messages.address')</span>
                        <input id="search-alamat" placeholder="@lang('messages.search')"
                            value="{{ session('mitra_alamat_tinggal') == '_' ? '' : session('mitra_alamat_tinggal') }}"
                            class="text-sm pl-28 pr-6 pt-1.5 pb-2 appearance-none rounded-md border block w-full bg-primary-20 border-primary-100 placeholder-gray-400 text-gray-700" />
                    </div>

                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100">@lang('messages.phonenumber')</span>
                        <input id="search-telpon" placeholder="@lang('messages.search')"
                            value="{{ session('mitra_telpon') == '_' ? '' : session('mitra_telpon') }}"
                            class="text-sm pl-28 pr-6 pt-1.5 pb-2 appearance-none rounded-md border block w-full bg-primary-20 border-primary-100 placeholder-gray-400 text-gray-700" />
                    </div>
                </div>
            </div>
        ...
        <div>
            @can('pegawai-create')
                <x-anchor-primary href="{{ route('mitra.create') }}">
