@if ($details->count() > 0)
    @foreach ($details as $detail)
        <tr>
            <td class="align-top">
                <x-text-span>{{ $detail->jenis_gantibaru->nama }}</x-text-span>
            </td>
            ...
            <td class="align-top text-right field-large-show">
                <x-text-span>{{ $detail->harga_satuan > 0 ? number_format($detail->harga_satuan, 0, ',', '.') : '-' }}</x-text-span>
            </td>
            <td class="align-top">
            ...
            <td class="align-top text-right">
                <div class="flex flex-row gap-1">
                    <x-text-span>{{ $detail->kuantiti > 0 ? $detail->kuantiti : '-' }}</x-text-span>
                    <x-text-span>{{ $detail->stock > 0 ? $detail->stock : '-' }}</x-text-span>
                </div>
            </td>
            <td class="align-top text-right field-large-show">
