    ...
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option {{ session('repair_pp') == config('custom.list_per_page_opt_1') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_1') }}">
                                {{ config('custom.list_per_page_opt_1') }}</option>
                            <option {{ session('repair_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option {{ session('repair_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:bg-primary-900 dark:border-primary-800 dark:text-gray-400 focus:border-primary-600 focus:ring-primary-600 focus-visible:ring-primary-600 focus:outline-none">
                            <option {{ session('repair_isactive') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('repair_isactive') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('repair_isactive') == '0' ? 'selected' : '' }} value="0">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-tanggal" type="date" placeholder="@lang('messages.search')"
                            value="{{ session('repair_tanggal') == '_' ? '' : session('repair_tanggal') }}"
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="customer-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:bg-primary-900 dark:border-primary-800 dark:text-gray-400 focus:border-primary-600 focus:ring-primary-600 focus-visible:ring-primary-600 focus:outline-none">
                            <option {{ session('repair_customer_id') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            @foreach ($customers as $id => $name)
                                <option {{ session('repair_customer_id') == $id ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="petugas-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:bg-primary-900 dark:border-primary-800 dark:text-gray-400 focus:border-primary-600 focus:ring-primary-600 focus-visible:ring-primary-600 focus:outline-none">
                            <option {{ session('repair_petugas_repair_id') == 'all' ? 'selected' : '' }}
                                value="all">
                                @lang('messages.all')</option>
                            @foreach ($petugass as $id => $name)
                                <option {{ session('repair_petugas_repair_id') == $id ? 'selected' : '' }}
                                    value="{{ $id }}">{{ $name }}</option>
                            @endforeach
                        </select>
                        <div
        ...
        <div class="pr-1">
            @can('maintenance-create')
                <x-anchor-primary href="{{ route('repair.create') }}">
