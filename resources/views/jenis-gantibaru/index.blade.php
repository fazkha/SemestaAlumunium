    ...
    <div class="mx-auto px-4 py-2">
        <div class="flex flex-col items-center">
            ...
            <div class="w-full overflow-x-auto">
                @include('jenis-gantibaru.partials.filter')
            </div>

            <div id="table-container" class="w-full">
                @include('jenis-gantibaru.partials.table')
            </div>
        ...
        <script type="text/javascript">
            $("#pp-dropdown, #isactive-dropdown, #search-nama").on(
                ...
                function() {
                    ...
                    var newURL = '{{ url('/service/replacement-type') }}';
                    var newState = {
                        page: 'index-replacement-type',
                    };
                    var newTitle = '{{ __('messages.gantibaru') }}';

                    window.history.pushState(newState, newTitle, newURL);

                    $.ajax({
                        url: '{{ url('/service/replacement-type/fetchdb') }}' + "/" + xpp + "/" + xisactive + "/" +
                            xnama,
                        type: "GET",
                        dataType: 'json',
