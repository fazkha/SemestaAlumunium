    ...
    <div class="mx-auto px-4 py-2">
        <div class="flex flex-col items-center">
            ...
            <div id="table-container" class="w-full">
                @include('jenis-perawatan.partials.table')
            </div>
        ...
        <script type="text/javascript">
            $("#pp-dropdown, #isactive-dropdown, #search-nama").on(
                ...
                function() {
                    ...
                    var newURL = '{{ url('/service/maintenance-type') }}';
                    var newState = {
                        page: 'index-maintenance-type',
                    };
                    var newTitle = '{{ __('messages.maintenancetype') }}';

                    window.history.pushState(newState, newTitle, newURL);

                    $.ajax({
                        url: '{{ url('/service/maintenance-type/fetchdb') }}' + "/" + xpp + "/" + xisactive + "/" +
                            xnama,
                        type: "GET",
                        dataType: 'json',
