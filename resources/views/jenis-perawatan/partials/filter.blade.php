<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
        <div
            ...
            <div class="flex flex-col lg:flex-row justify-start">
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option
                                {{ session('maintenance-type_pp') == config('custom.list_per_page_opt_1') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_1') }}">
                                {{ config('custom.list_per_page_opt_1') }}</option>
                            <option
                                {{ session('maintenance-type_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option
                                {{ session('maintenance-type_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:bg-primary-900 dark:border-primary-800 dark:text-gray-400 focus:border-primary-600 focus:ring-primary-600 focus-visible:ring-primary-600 focus:outline-none">
                            <option {{ session('maintenance-type_isactive') == 'all' ? 'selected' : '' }}
                                value="all">
                                @lang('messages.all')</option>
                            <option {{ session('maintenance-type_isactive') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('maintenance-type_isactive') == '0' ? 'selected' : '' }} value="0">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 rounded-md border border-primary-100 dark:bg-primary-900 dark:border-primary-800">@lang('messages.maintenance_2')</span>
                        <input id="search-nama" placeholder="{{ __('messages.search') }}"
                            value="{{ session('maintenance-type_nama') == '_' ? '' : session('maintenance-type_nama') }}"
        ...
        <div>
            @can('inspect-create')
                <x-anchor-primary href="{{ route('maintenance-type.create') }}">
